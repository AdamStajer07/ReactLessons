...
function ImgWithTextTemplate({img, title, text, duration='left'}) {
  return (
    <div className='flex justify-between w-4/5 min-h-12 max-h-240 rounded-md overflow-hidden'>
        {duration == 'left' && <img src={img} className='object-cover hover:scale-125 transition-all'/>}
        ...
        {duration == 'right' && <img src={img} className='object-cover hover:scale-125 transition-all'/>}
    </div>
  )
}
